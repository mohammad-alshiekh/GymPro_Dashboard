...
import {
  Plus, Pencil, Trash2, Eye, Wrench, AlertCircle, RefreshCw,
} from 'lucide-react';
...
function formFromEquipment(mg: Equipment): EquipmentPayload {
  return {
    nameEn: mg.nameEn,
    nameAr: mg.nameAr,
...
export default function EquipmentPage() {
  ...
  const loadList = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await listEquipment(accessToken, {
        pageNumber: page,
      ...
      setError(err instanceof Error ? err.message : 'Failed to load equipment items');
    } finally {
      setLoading(false);
    }
  ...
  const openEdit = (mg: Equipment) => {
    setForm(formFromEquipment(mg));
    setFormError(null);
    setFormModal({ open: true, mode: 'edit', id: mg.id });
  };

  const openDetail = async (mg: Equipment) => {
    if (!accessToken) return;
    setDetail(mg);
    setDetailLoading(true);
    try {
      const fresh = await getEquipment(accessToken, mg.id);
      setDetail(fresh);
    } catch {
  ...
  const handleSave = async () => {
    ...
    try {
      if (formModal.mode === 'add') {
        await createEquipment(accessToken, payload);
        showSuccess('Equipment created successfully.');
        setPage(1);
      } else if (formModal.id) {
        await updateEquipment(accessToken, formModal.id, payload);
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await deleteEquipment(accessToken, deleteTarget.id);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Define and maintain equipment items used across the exercise library.
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {loading ? (
          ...
          <>
            <StatCard title="Total items" value={totalCount} icon={<Wrench size={22} />} color="indigo" />
            <StatCard title="On this page" value={items.length} icon={<Wrench size={22} />} color="blue" />
            <StatCard title="Active (page)" value={activeOnPage} icon={<Wrench size={22} />} color="emerald" />
          </>
        )}
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200/80 overflow-hidden">
        ...
        {!loading && filteredItems.length === 0 && (
          <div className="py-16 text-center">
            ...
            <p className="text-gray-600 font-medium">No equipment items found</p>
            <p className="text-sm text-gray-400 mt-1 max-w-sm mx-auto">
              {debouncedSearch
                ? 'Try a different search term or clear filters.'
                : 'Create your first equipment item to organize exercises.'}
            </p>
            {!debouncedSearch && (
              <button
                type="button"
      ...
      {/* Detail modal */}
      <Modal isOpen={!!detail} onClose={() => setDetail(null)} title="Equipment Details" size="lg">
        {detail && (
          <div className="space-y-6">
            {detailLoading && (
      ...
      <Modal
        ...
        title={formModal.mode === 'add' ? 'Add Equipment' : 'Edit Equipment'}
        size="lg"
      >
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <textarea
                ...
                rows={4}
                placeholder="Standard barbell used for compound lifts…"
          ...
          <label className="flex items-center gap-3 cursor-pointer">
            ...
            />
            <span className="text-sm font-medium text-gray-700">Active — visible in exercise definitions</span>
          </label>
      ...
      {/* Delete modal */}
      <Modal isOpen={!!deleteTarget} onClose={() => !deleting && setDeleteTarget(null)} title="Delete Equipment" size="sm">
        {deleteTarget && (
          <>
            ...
            <p className="text-sm text-gray-400 mb-6">
              Exercises using this equipment may need to be updated. This cannot be undone.
            </p>
